...
# Проверка сущ-я спрайта и его возврат
...
class Tetris:
    ...
    def render_cup(self, screen): # Рисуем стакан, в дальнейшем надо будет удалить внутр. квадраты, но пока не надо, т.к. так удобнее
        pygame.draw.rect(screen, pygame.Color(255, 255, 255),
                         (self.left, self.top, self.size_square * 10 + 1, self.size_square * 20 + 1), 2)
...
#                    max_y_o = i * 30
    ...
    def burn_line(self): # Сгорание фигур
        burning_line = []
    ...
    def draw_interface(self, screen, size_screen, size_square, inp_score, inp_timer, inp_level): # Отрисовка интерфейса
        color = (255, 255, 255)
    ...
    # Если проиграет, то окно поражения
...
def main():
    ...
    flag_start = True # Флаг отвечает за начальное окно с правилами
    flag_loss = True # Флаг отвечает за проигршное окно
    flag = False # тот самы флаг со строки 155
    figur_ver = 0 # выставляю начальное вариант фигуры

    figure = tetris.view_figure() # Созадние рандомной фигуры
    next_figure = tetris.view_figure() # Создание след. фигуры
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:  # движение фигуры влево и вправо
                if not flag_loss and event.key == pygame.K_RIGHT and tetris.examination_bottom_coords(figure, pos_x_figure + size_square, # Проверяет, можно ли фигуре переметится впарво
                    ...
                    if hold_figur_for_sprite != -1 and not flag_loss: # Запас фигуры
                    ...
                    if flag_loss: # Если игрок проиграл
                        if timer != '0': # Записывает в файл результат
        ...
        if flag_loss and not flag_start: # Отрисовка экрана поражения
        ...
        if not flag_start: # Если не начала игры
            # Отрисовка
